...
const Navbar = () => {
  ...
  return (
    <div className="z-10 flex fixed top-0 w-full items-start justify-between">
      <div className="lg:w-28 p-3 sm:w-14 w-14">
        <Link to="/">
          <svg
            className="w-full"
      ...
      <button
        ...
        className="relative lg:h-12 lg:w-40 sm:h-8 sm:w-28 h-4 w-14 text-white bg-black group overflow-hidden hover:text-black"
      >
        <div className="bg-lime-300 absolute top-0 h-0 w-full transition-all duration-300 group-hover:h-full"></div>

        <div className="absolute right-0 bottom-0 lg:p-3 lg:pr-6 flex flex-col lg:text-sm sm:text-xs text-[4px] lg:leading-2.5 sm:leading-2 leading-1 sm:p-2 sm:pr-3 p-1 pr-1 text-right">
          <p>─────────</p>
          <p>────</p>
        </div>
      </button>
